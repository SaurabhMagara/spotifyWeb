...
const Albums = ({ passData }: { passData: string }) => {
  ...
  const getSampleData = async () => {
    ...
    try {
      ...
      const url = `${passData === "new-releases" ? "http://localhost:5000/api/v1/" + passData
        : passData === "albums" ? "http://localhost:5000/api/v1/" + passData + "/90shindi"
  ...
  return (
    <div className=" flex items-center min-h-screen w-screen flex-col gap-5 bg-gray-950 subpixel-antialiased overflow-y-hidden pb-4">
      ...
      <div className="flex justify-between items-center w-3/4 px-5">
        <h1 className="text-gray-200 font-bold text-5xl text-center">{passData === "new-releases" ? "New Releases" : passData === 'artist' ? "Artist" : "Albums"}</h1>
        {
...
export const AlbumCard = ({ url, name, followers, tracks, passData, spotify, popularity }: propData) => {
  return (
    <div className={`flex flex-col gap-3 justify-center items-center rounded-lg bg-gradient-to-tl from-gray-900 to-gray-800 shadow-md group transition-transform duration-300 hover:scale-105 overflow-hidden text-lg pb-2 hover:cursor-pointer`}>
