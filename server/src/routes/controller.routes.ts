...
export const getAccessToken = async (req: express.Request, res: express.Response) => {
    try {
        ...
        console.log(error);
        res.status(500).json({ message: "error while getting token", err: error })
    }
}
...
export const getCategories = async (req: express.Request, res: express.Response) => {
    try {
        ...
        console.log(err);
        res.status(500).json({ msg: "category error", error: err });
    }
}

export const getArtists = async (req: express.Request, res: express.Response) => {
    try {
        const { token } = req.body;
        ...
        res.status(200).json({ message: "artists recieved.", artists: artists });
    } catch (error) {
        console.log(error);
        res.status(500).json({ message: "getArtists error", error: error })
    }
}

export const getAlbumsOfArtist = async (req: express.Request, res: express.Response) => {
    try {
        const { token } = req.body;
        ...
        res.status(200).json({ message: "albums recieved", albums: albums });

    } catch (error) {
        console.log(error);
        res.status(500).json({ message: "getAlbum err", errro: error });
    }
}

export const getArtistsTopTracks = async (req : express.Request, res : express.Response)=>{
    try {
        const {token} = req.body;
        const {name} = req.query;
        ...
        res.status(200).json({message : "top tracks recieved", tracks : tracks});

    } catch (error) {
        console.log(error);
        res.status(500).json({message : "artistsTopTrack err", error : error})
    }
}

export const getNewReleases = async(req: express.Request, res : express.Response)=>{
    try {
        const {token} = req.body;
        ...
        res.status(200).json({message: "new Released albums recieved.", albums : albums});
    } catch (error) {
        console.log(error);
        res.status(500).json({messsage : "getNewReleases err", error : error});
    }
}

export const getTracksOfAlbum = async(req : express.Request, res : express.Response)=>{
    try {
        const {token} = req.body;
        ...
        res.status(200).json({message:"tracks recieved", tracks : tracks});
    } catch (error) {
        console.log(error);
        res.status(500).json({message : "tracksOfAlbum err", error : error});
    }
}